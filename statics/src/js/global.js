(function (global) {
  ...
  'use strict';
  var requires = ['LocalStorageModule', 'jt.service.const',
    'jt.service.debug', 'jt.service.httpLog', 'jt.service.user',
    'jt.directive.widget'
  ];
  ...
  app.run(['$http', '$timeout', '$window', 'localStorageService', 'CONST',
    'debug', run
  ]);
  ...
  function run($http, $timeout, $window, localStorageService, CONST, debug) {
    ...
    TIMING.start('app');
    TIMING.start('window');
    var appEndTimer = setTimeout(function (argument) {
      appEndTimer = 0;
      TIMING.end('app');
    }, 0);
    debug = debug('app.run');
